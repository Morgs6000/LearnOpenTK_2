...
public class Program
{
    ...
    private unsafe static void Main(string[] args)
    {
        ...
        // --------------------------------------------------
        Shader asteroidShader = new Shader("src/asteroids.vs", "src/asteroids.fs");
        Shader planetShader = new Shader("src/planet.vs", "src/planet.fs");
        ...
        GL.BufferData(BufferTarget.ArrayBuffer, (int)(amount * sizeof(Matrix4)), ref modelMatrices[0], BufferUsageHint.StaticDraw);
        ...
        while (!GLFW.WindowShouldClose(window))
        {
            ...
            GL.BindTexture(TextureTarget.Texture2D, rock.textures_loaded[0].id); // nota: também tornamos público (em vez de privado) o vetor textures_loaded da classe model.

            for (int i = 0; i < rock.meshes.Count(); i++)
            {
                GL.BindVertexArray(rock.meshes[i].VAO);
                GL.DrawElementsInstanced(PrimitiveType.Triangles, rock.meshes[i].indices.Count(), DrawElementsType.UnsignedInt, 0, (int)amount);
                GL.BindVertexArray(0);
            }
